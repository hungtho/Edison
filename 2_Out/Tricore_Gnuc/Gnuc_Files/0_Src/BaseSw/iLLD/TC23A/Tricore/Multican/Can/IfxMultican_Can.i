...
# 1 "<command-line>"
# 1 "0_Src/BaseSw/iLLD/TC23A/Tricore/Multican/Can/IfxMultican_Can.c"
# 49 "0_Src/BaseSw/iLLD/TC23A/Tricore/Multican/Can/IfxMultican_Can.c"
...
# 50 "0_Src/BaseSw/iLLD/TC23A/Tricore/Multican/Can/IfxMultican_Can.c" 2
...
IfxMultican_Status IfxMultican_Can_MsgObj_init(IfxMultican_Can_MsgObj *msgObj, const IfxMultican_Can_MsgObjConfig *config)
{
    ...
    boolean longFrame = (config->control.messageLen > IfxMultican_DataLengthCode_8) ? (1u) : (0u);
    ...
    boolean gatewaySourceObj = (config->gatewayTransfers != 0) ? (1u) : (0u);


    boolean receiveFifo = (0u), transmitFifo = (0u);
    ...
    }
    {
        ...
        if (config->node->fastNode && longFrame)
        {
            ...
            IfxMultican_MsgObj_setExtendedDataLength(hwObj, (1u));
        ...
        else
        {
            ...
            IfxMultican_MsgObj_setSingleTransmitTrial(hwObj, (0u));


            IfxMultican_MsgObj_setSingleDataTransfer(hwObj, (0u));
        }
    }
...
void IfxMultican_Can_MsgObj_initConfig(IfxMultican_Can_MsgObjConfig *config, IfxMultican_Can_Node *node)
{
    ...
    config->control.fastBitRate = (0u);

    config->control.extendedFrame = (0u);
    config->control.matchingId = (0u);
    config->control.singleDataTransfer = (0u);
    config->control.singleTransmitTrial = (0u);
    config->acceptanceMask = 0x7FFFFFFFUL;
    config->messageId = 0;

    config->frame = IfxMultican_Frame_receive;
    config->priority = IfxMultican_Priority_CAN_ID;
    config->rxInterrupt.enabled = (0u);
    config->rxInterrupt.srcId = IfxMultican_SrcId_0;
    config->txInterrupt.enabled = (0u);
    config->txInterrupt.srcId = IfxMultican_SrcId_0;

    config->firstSlaveObjId = 0;

    config->gatewayTransfers = (0u);
    config->gatewayConfig.copyDataLengthCode = (1u);
    config->gatewayConfig.copyData = (1u);
    config->gatewayConfig.copyId = (1u);
    config->gatewayConfig.enableTransmit = (1u);
...
void IfxMultican_Can_Node_getConfig(IfxMultican_Can_Node *node, IfxMultican_Can_NodeConfig *config)
{
    ...
    config->loopBackMode = (hwNode->PCR.B.LBM != 0) ? (1u) : (0u);
    config->analyzerMode = (hwNode->CR.B.CALM != 0) ? (1u) : (0u);
}
...
IfxMultican_Status IfxMultican_Can_Node_init(IfxMultican_Can_Node *node, const IfxMultican_Can_NodeConfig *config)
{
...

    {
        ...
        {
            IfxMultican_Node_setFastNode(hwNode, (1u));
            node->fastNode = (1u);
            ...
            if (config->fdConfig.loopDelayOffset)
            {
                ...
                IfxMultican_Node_setTransceiverDelayCompensation(hwNode, (1u));
            }
        }
    }
...
void IfxMultican_Can_Node_initConfig(IfxMultican_Can_NodeConfig *config, IfxMultican_Can *mcan)
{
    ...
    config->loopBackMode = (0u);
    config->analyzerMode = (0u);

    config->baudrate = 500000;
    config->samplePoint = 8000;
    config->synchJumpWidth = 2000;

    config->flexibleDataRate = (0u);
...
IfxMultican_Status IfxMultican_Can_initModule(IfxMultican_Can *mcan, const IfxMultican_Can_Config *config)
{
    ...
    if (config->clockSelect == IfxMultican_ClockSelect_fclc)
    {
        ...
        while (IfxMultican_isModuleEnabled(mcanSFR) == (0u))
        {}
    ...
    for (srcId = IfxMultican_SrcId_0; srcId < (16); srcId++)
    {
